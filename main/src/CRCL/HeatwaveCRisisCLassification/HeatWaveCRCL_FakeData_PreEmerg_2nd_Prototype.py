...
def CrisisClassificationHeatwave_FakeData_PreEmerg_v2(iteration, flag_location_status, flag_HOCL, CategThresh):

    ver = 'PreEmerg_FakeData_2nd_PROTOTYPE_AUTORUN'
    ...
    folder_name = "TOPICS_HW_Pre_EMERG_FakeData_" + str(now.year) + "_" + str(now.month) + "_" + str(now.day) + "/"
    ...
    # Fake Data
    # *************************** read an already xlsx for standard values patch ********************
    # data for ver 9
    fl_xlsx_path = root_path_dir + 'Weather_Simulated_Data_perHour_v2.xlsx'

    dset = pd.read_excel(fl_xlsx_path)

    print("iteration = ", iteration, "...utc current date=", utc_current_date)

    # Convert the DateTime column of the dset to the current DateTime starting from the Time 00:00:00 of the
    # current date
    newdata = convert_to_current_date_time(dset, utc_current_date)

    # find the nearest date/time to a current date/time (in utc format) and return the entries
    # with date/time greater than the current one
    result = find_nearest_df_to_current_hour(newdata, utc_current_date_time)

    # Store data frame to xlsx file
    dfxls = pd.ExcelWriter(directory + "/" + "Simulated_Data_perHour_V2.xlsx")
    ...
    print(" STEP 3: Calculates the Disconfort Index (DI)...")
