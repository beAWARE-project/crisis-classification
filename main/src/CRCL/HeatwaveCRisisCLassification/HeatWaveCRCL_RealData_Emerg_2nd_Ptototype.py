...
# Modified Date:
#
# -------------------------------- 2nd PROTOTYPE VERSION - REAL DATA EMERGENCY PHASE AUTORUN --------------------------------------------------
#
#   During the Heatwave crisis. Real-time weather data from DarkSky
#
...
def CrisisClassificationHeatwave_RealData_Emerg_v2(iteration, flag_location_status, flag_HOCL, CategThresh):

    ver = 'Emerg_RealData_2nd_PROTOTYPE_AUTORUN'
    ...
    now = datetime.now()
    folder_name = "TOPICS_HW_EMERG_RealData_" + str(now.year) + "_" + str(now.month) + "_" + str(now.day) + "/"
    ...
    print("\n STEP 1: Fetch real-time observation data from the Dark Sky API ...")

    darksky_addr = 'https://api.darksky.net/forecast/'
    my_api_key = '1f026c3f7f3d38e626838dbb686d72b7'

    exclude_parameters = ["minutely", "hourly", "daily", "alerts", "flags"]

    # Call function to create query list
    qrlist = extract_realtime_weather_darksky(darksky_addr, my_api_key, exclude_parameters, points)
    ...
    # Real-time weather data acquisition from query list to json
    realtime_weather_df = parse_json_to_dict_querylist(qrlist, points, directory)

    # print(realtime_weather_df)
    ...
    # For Step 2
    DSDI = Heatwave_Disconfort_Index(realtime_weather_df, InitTemp, model_param)
