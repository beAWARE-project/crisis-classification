...
# Modified Date:
#
# -------------------------------- 2nd PROTOTYPE VERSIONS - REAL-TIME FORECASTS BASED ON VICENZA PILOT -------------------------------------------
#       Version with:
#                     - Real time forecasts from SensorThings Server via AMICO runs
#                     - Risk Maps
#
...
#---------------------------------------------------------------------------------------------------------------
def CrisisClassificationFlood_PreEmerg_RealData_v2(cur_date, flag_mode, time_mode, flag_last_run):

    tp6 = 0
    tp41 = 0
    tp42 = 0
    ver = 'PreEmerg_RealData_2nd_PROTOTYPE'
    ...
    directory = root_path_dir + ver + "_" + "TOPICS" + "_" + flag_mode + "_" + time_mode + "_" + cur_date.isoformat()
    ...
    if time_mode == 'Current':
        ...
        # Extract from STS the data from the real case Flood in 2010
        dates = ['2010-10-31T00:00:00.000Z', '2010-11-03T00:00:00.000Z']


    print(" <<< Testing Reasons - Dates: \n")
    print(" <<<< ", dates, " >>>> \n")
    #
    ...
    for counter in range(0, count):
        ...
        if len( IntRS[ IntRS.ix[:,'Name'].str.contains(riverSections["value"][counter]['name']) ] ) != 0:
            ...
            if flag_last_run == False:
                response_forecast = extract_forecasts(str(service_root_URI), SensorThings, ids, sel_vals, ord_vals,
                                                      filter_args=filt_args, filter_vals=filt_vals)
            else:
                print(" --> Execute query for lastRun regardless DateTime attribute. \n")
                response_forecast = extract_forecasts(str(service_root_URI), SensorThings, ids, sel_vals, ord_vals,
                                                      last_run=flag_last_run, filter_args=filt_args, filter_vals=filt_vals)
                ...
                if flag_extreme == True:
                    ...
                    if resp_comparison['Measurement_Color'] != '#31A34F':
                        ...
                        if len(response_q['features']) > 0:
                            ...
                            # Create Top106 for risk maps - polygons
    ...
    print(
        " Time for Step 2. Calculate WL scale create Topics 104/106 for River Sections and for Risk Maps, and send them to the PSAP : ",
        Calculations_Topics, " seconds")
    ...
    Timers_lists = [['Data Acquisition', Data_Acquisition, Data_Acquisition_min],
                    ['WL and Topics', Calculations_Topics, Calculations_Topics_min],
                    ['Risk Maps', Polygon_Time, Polygon_Time_min],
                    ['Overalls OCCI/PFLCL', Overalls_Time, Overalls_Time_min],
